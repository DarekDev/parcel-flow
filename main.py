#!/usr/bin/env python3
"""
ParcelFlow command-line interface.

A didactic demonstration of the data-driven execution model: nodes run when the
data they require is available, with no static execution order.
"""
...
from workflows import (
    create_simple_workflow, create_independent_workflow, create_array_workflow,
...
def run_workflow(workflow_name: str, verbose: bool = False):
    ...
    if workflow_name == "simple":
        ...
        nodes = create_independent_workflow()
    ...
    print(f"\nExecution completed in {len(engine.get_execution_log())} log steps")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ParcelFlow - data-driven workflow execution",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py simple          # Run the simple linear workflow
  python main.py independent     # Run the independent-branches workflow
  python main.py array           # Run the array spreading (scatter/gather) workflow
